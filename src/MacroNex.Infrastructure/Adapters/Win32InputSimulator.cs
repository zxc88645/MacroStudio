...
public class Win32InputSimulator : IInputSimulator
{
    ...
    public async Task SimulateKeyPressAsync(VirtualKey key, bool isDown)
    {
        ...
        await Task.Run(() =>
        {
            lock (_lockObject)
            {
                // ‰ΩøÁî® scan code + KEYEVENTF_SCANCODE ‰æÜÊ®°?¨ÂØ¶È´îÈçµ?§Ëº∏?•Ô?
                // ?ê‰??äÊà≤Â∞çÈÄôÁ®Æ?πÂ??ÑÊîØ?¥Ê?ÊØîÂ?‰ΩøÁî®?õÊì¨?µÁ¢º?¥Â•Ω??
                ...
                var input = new INPUT
                {
                    ...
                    u = new InputUnion
                    {
                        ki = new KEYBDINPUT
                        {
                            // ‰ΩøÁî® scan code Ê®°Â??ÇÔ?wVk ‰∏Ä?¨Ë®≠??0ÔºåÁî± wScan + dwFlags Ê±∫Â?ÂØ¶È??µ„Ä?
                            wVk = 0,
                            wScan = scanCode,
    ...
    /// <summary>
    /// ?πÊ??õÊì¨?µÂ?ÂæóÂ??âÁ? scan code ?áÂ?Ë¶ÅÁ??óÊ? (Â¶?EXTENDEDKEY)Ôº?
    /// ‰∏¶Â?‰∏?KEYEVENTF_SCANCODE ËÆ?SendInput ‰ª•Ê??èÁ¢ºÊ®°Â??ÅÂá∫??
    /// </summary>
    private static (ushort scanCode, uint flags) GetScanCodeAndFlags(uint virtualKey)
    {
        // ‰ΩøÁî®?ÆÂ??µÁõ§?çÁΩÆ‰æÜÂ?ËΩâÊ?
        var layout = GetKeyboardLayout(0);
        var scanCode = (ushort)MapVirtualKeyEx(virtualKey, MAPVK_VK_TO_VSC, layout);

        uint flags = KEYEVENTF_SCANCODE;

        // ?ê‰??µÔ?‰æãÂ??πÂ??µ„ÄÅInsert?ÅDelete?ÅHome?ÅEnd?ÅPageUp/Down Á≠âÔ??ÄË¶?EXTENDEDKEY
        // ?ôË£°?®Â∏∏Ë¶ãÁ? VK ?ºÂ?Á∞°ÂñÆ?§Êñ∑ÔºõËã•?™ÂëΩ‰∏≠Â??™‰Ωø?®Ê??èÁ¢º??
        switch (virtualKey)
        {
            // ÁÆ≠È†≠??
            case 0x25: // VK_LEFT
