...
public class Win32InputSimulator : IInputSimulator
{
    ...
    public async Task SimulateKeyPressAsync(VirtualKey key, bool isDown)
    {
        ...
        await Task.Run(() =>
        {
            lock (_lockObject)
            {
                // 使用 scan code + KEYEVENTF_SCANCODE 來模?�實體鍵?�輸?��?
                // ?��??�戲對這種?��??�支?��?比�?使用?�擬?�碼?�好??
                ...
                var input = new INPUT
                {
                    ...
                    u = new InputUnion
                    {
                        ki = new KEYBDINPUT
                        {
                            // 使用 scan code 模�??��?wVk 一?�設??0，由 wScan + dwFlags 決�?實�??��?
                            wVk = 0,
                            wScan = scanCode,
    ...
    /// <summary>
    /// ?��??�擬?��?得�??��? scan code ?��?要�??��? (�?EXTENDEDKEY)�?
    /// 並�?�?KEYEVENTF_SCANCODE �?SendInput 以�??�碼模�??�出??
    /// </summary>
    private static (ushort scanCode, uint flags) GetScanCodeAndFlags(uint virtualKey)
    {
        ...
        // ?��??��?例�??��??�、Insert?�Delete?�Home?�End?�PageUp/Down 等�??��?EXTENDEDKEY
        // ?�裡?�常見�? VK ?��?簡單?�斷；若?�命中�??�使?��??�碼??
        switch (virtualKey)
        {
            // 箭頭??
            case 0x25: // VK_LEFT
