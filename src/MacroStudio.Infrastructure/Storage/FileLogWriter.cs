...
public class FileLogWriter : IFileLogWriter
{
    ...
    public async Task ClearLogsAsync()
    {
        lock (_fileLock)
        {
            try
            {
                ...
                var logFiles = Directory.GetFiles(_logDirectory, "macrostudio*.log");
    ...
    public async Task ExportLogsAsync(string filePath, IEnumerable<LogEntry> entries)
    {
        ...
        try
        {
            ...
            lines.Add("MacroStudio Log Export");
    ...
    private void RotateLogFiles()
    {
        try
        {
            // Rename existing files (macrostudio.log -> macrostudio.1.log, etc.)
            for (int i = MaxLogFiles - 1; i >= 1; i--)
            {
                var oldFile = Path.Combine(_logDirectory, $"macrostudio.{i}.log");
                var newFile = Path.Combine(_logDirectory, $"macrostudio.{i + 1}.log");
            ...
            {
                var firstRotated = Path.Combine(_logDirectory, "macrostudio.1.log");
            ...
            var oldestLog = Path.Combine(_logDirectory, $"macrostudio.{MaxLogFiles + 1}.log");
